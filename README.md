...
| `DrawablyPicker` | A pen chevron opening a sketched list, tailed back to the field — no platform chrome around it. The box is pre-sized to the widest option so picking never shifts the layout. |
...
The showcase's app icon is a lowercase "d" drawn by the library's own stroke
engine — the same `Rough.circle` and `Rough.line` a button's border uses, at a
pinned seed. Regenerate it with `node Tools/gen-icon.mjs emit`.
